    ...
    for (var i = 0; i < sorted.length; i++) {
      ...
      cardBody.innerHTML =
        ...
        '<div class="hc-info">' +
           ...
           '</div>' +
           (function() { var _w = Service.getWeapon(hid); if (!_w) return '<div class="hc-weapon">' + getWeaponAttackStr(HERO_WEAPON[hd.type]) + ' ' + hd.desc + '</div>'; var _qn = WEAPON_QUALITY[_w.quality] ? WEAPON_QUALITY[_w.quality].name : '?'; var _qc = WEAPON_QUALITY[_w.quality] ? WEAPON_QUALITY[_w.quality].color : '#888'; var _wt = WEAPON_TYPE_LABELS[_w.type] || '?'; var _s = '<span style="color:' + _qc + ';font-weight:bold;">[' + _qn + _wt + ']</span> ⚔+' + _w.atkPct + '%'; if (_w.hpPct) _s += ' ❤+' + _w.hpPct + '%'; if (_w.spd) _s += ' 🏃+' + (_w.spd || 0).toFixed(2); if (_w.extraSkill) _s += '<br><span style="font-size:10px;color:#ffd700;">⭐' + _w.extraSkill.name + ' (' + _w.extraSkill.procRate + '%): ' + _w.extraSkill.desc + '</span>'; return '<div class="hc-weapon">' + _s + '</div>'; })() +
           (bondsHtml ? '<div class="hc-bonds">' + bondsHtml + '</div>' : '') +
            (hd.skill ? '<div class="hc-skill" style="margin-top:4px;font-size:12px;color:#ffd700;">\u2726\u5927\u62db \u3010' + hd.skill.name + '\u3011: ' + (hd.skill.getDesc ? hd.skill.getDesc(hd.rarity) : hd.skill.desc) + ' (CD: ' + hd.skill.cd + '\u79d2)</div>' : '') +
         '</div>';
