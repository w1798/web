const links = [
    ...
    { id: "ClassSeatPlanner", category: "daily", title: "排座智慧家", desc: "　是一款專為導師設計的智慧化座位管理系統。它突破傳統手寫限制，提供自定義「禁用區域」與一鍵「隨機編排」。系統核心具備「十字」與「九宮格」衝突避讓演算法，能精準確保特定學生不相鄰。透過直觀的拖拽操作與即時位子標記功能，讓複雜的座位調整變得迅速且公平，是優化課堂秩序、提升行政效率的最佳數位幫手。", url: "ClassSeatPlanner" }, 
    { id: "Chiefly", category: "daily", title: "各司其職 ", desc: "　是高效班級職務分配系統，支援多工作表、智慧隨機分配與直覺拖放。具備單/多選模式、高度客製排版與字體，並透過 GZIP 壓縮技術實現檔案級備份，助您輕鬆管理班級領導分配。", url: "Chiefly" }, 
    ...
    { id: "E-magazine", category: "stage", title: "電子校刊作品管理", desc: "　整合兩大行政流程：一是將學生作文或童詩的 Word 檔解析後，自動排版並匯出格式化文件；二是依班級名單、流水號或手動清單批次產生改名批次檔（.bat），支援排序模式與一鍵還原，大幅提升校刊編輯的處理效率。", url: "E-magazine" },
    ...
    { id: "GutTracker", category: "tool", title: "順暢日記", desc: "　專為追蹤飲食與排便關聯設計。透過核心「位移對齊」分析技術，將瑣碎紀錄轉化為視覺化規律，助您精準掌握身體與飲食間的因果關係。結合現代化介面，提供最科學的個人化照護，讓守護腸道健康變得直觀且優雅。", url: "GutTracker" },            
...
const CATEGORY_CONFIG = {
    all: { label: '全部彙整', filter: () => true },
    fav: { label: '我的最愛', filter: (item, favs) => favs.includes(item.id) },
...
function renderSidebar() {
    ...
    navGroup.innerHTML = Object.keys(CATEGORY_CONFIG).map(key => {
        const isActive = currentCategory === key ? 'active' : '';
        return `<div id="nav-${key}" class="nav-item ${isActive}" onmouseenter="filterLinks('${key}', this)">${CATEGORY_CONFIG[key].label}</div>`;
    }).join('');
}
...
function navigateCategory(direction) {
    ...
    const cats = ['all', 'fav', 'daily', 'stage', 'doc', 'game', 'tool'];
...
function updateSidebarCounts() {
    ...
    Object.keys(CATEGORY_CONFIG).forEach(cat => {
        ...
        if (el) {
            let count = 0;
            if (cat === 'all') count = links.length;
            else if (cat === 'fav') count = validFavorites.length;
            else count = links.filter(l => l.category === cat).length;
            
            el.innerText = `${CATEGORY_CONFIG[cat].label}(${count})`;
        }
    });
}
